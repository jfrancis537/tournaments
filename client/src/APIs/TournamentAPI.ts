...
import { SerializedTournament, Tournament, TournamentMetadata, TournamentOptions, TournamentState } from "@common/Models/Tournament";
...
export namespace TournamentAPI {
  ...
  export async function setTournamentMetadata(metadata: TournamentMetadata) {
    const resp = await fetch(`${TournamentAPIConstants.BASE_PATH}${TournamentAPIConstants.SET_METADATA(metadata.id)}`, {
    ...
    if (!resp.ok) {
      throw new HttpStatusError("Error occured while setting tournament metadata.", resp.status);
    }
  }

  export async function getTournamentMetadata(id: string) {
    const resp = await fetch(`${TournamentAPIConstants.BASE_PATH}${TournamentAPIConstants.GET_METADATA(id)}`,);

    if (resp.ok) {
      return await resp.json() as TournamentMetadata;
    } else if (resp.status === 404) {
      return undefined;
    } else {
      throw new HttpStatusError("Error occured while getting tournament metadata.", resp.status);
    }
